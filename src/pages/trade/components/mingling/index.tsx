...
export default function MingLing({ hyorders, nowTab }) {
  ...
  const getNode = () => {
        ...
        >
          <div className="listTitleCell-2">
            <div className="listTitleCell-3">
              ...
              <span className="listTitleCell-5">{data.num}</span>
    ...
    if (nodes.length === 0) {
      ...
      nodes.push(
        <div className="mingling-44">{translate(getText("暂无更多了"))}</div>
      );
    }
    return nodes;
  };
  return (
    <div role="tabpanel" id="mingling" className="mingling-1">
      ...
      <div className="mingling-2">{getNode()}</div>
    </div>
  );
}
