import { Empty } from "antd-mobile";
import { useTranslation } from "react-i18next";
import { useNavigate } from "react-router-dom";
import { getText } from "../../../../utils/util";
import "./index.css";

export default function MingXi({ hyorders, nowTab }) {
  const navigate = useNavigate();
  const { t: translate } = useTranslation();
  const tab = nowTab.toUpperCase() + "/USDT";

  const getNode = () => {
    hyorders = hyorders.filter((data) => data.coinname === tab);
    hyorders = hyorders.filter((data) => data.status === 2);
    const nodes = [];
    for (let index = 0; index < hyorders.length; index++) {
      const data = hyorders[index];
      const node = (
        // <div
        //   className="levermingxi-3"
        //   onClick={() => {
        //     const sendData = {
        //       id: data.id,
        //       coinname: data.coinname,
        //       num: data.num,
        //       ploss: data.ploss,
        //       buytime: data.buytime,
        //       status: data.status,
        //     };
        //     navigate(
        //       `/marketOrderInfo/${data.id}?data=${JSON.stringify(sendData)}`
        //     );
        //   }}
        // >
        //   <div className="levermingxi-4">
        //     <span className="levermingxi-5">
        //       <span
        //         className={data.hyzd === 1 ? "levermingxi-6" : "levermingxi-26"}
        //       >
        //         {data.hyzd === 1
        //           ? translate(getText("借多"))
        //           : translate(getText("空"))}
        //         <div className="levermingxi-7">
        //           {data.hyzd === 1 && (
        //             <img
        //               alt=""
        //               src="data:image/png;base64,"
        //               className="levermingxi-8"
        //             />
        //           )}
        //           {data.hyzd === 2 && (
        //             <img
        //               alt=""
        //               src="data:image/png;base64,"
        //               className="levermingxi-8"
        //             />
        //           )}
        //         </div>
        //       </span>
        //       <br className="levermingxi-9" />
        //       <h5 className="levermingxi-10">{data.coinname}</h5>
        //     </span>
        //   </div>
        //   <div className="levermingxi-11">
        //     <span className="levermingxi-12">
        //       {data.isWin === 1 ? data.ploss : -data.ploss}
        //     </span>
        //   </div>
        //   <div className="levermingxi-13">
        //     <span className="levermingxi-14">
        //       <span className="levermingxi-15">
        //         {translate(getText("投資金額"))}
        //         <br className="levermingxi-16" />
        //         {data.num}
        //       </span>
        //     </span>
        //     <br className="levermingxi-17" />
        //     <span className="levermingxi-18">
        //       {translate(getText("開盤價"))} <br className="levermingxi-19" />
        //       {data.buyprice}
        //     </span>
        //     <br className="levermingxi-20" />
        //     <span className="levermingxi-21">
        //       {translate(getText("收盤價"))} <br className="levermingxi-22" />
        //       {data.sellprice}
        //     </span>
        //   </div>
        // </div>
        <div
          className="listTitleCell-1"
          onClick={() => {
            const sendData = {
              orderNo: data.orderNo,
              coinname: data.coinname,
              num: data.num,
              ploss: data.ploss,
              buytime: data.buytime,
              status: data.status,
            };
            navigate(
              `/marketOrderInfo/${data.id}?data=${JSON.stringify(sendData)}`
            );
          }}
        >
          <div className="listTitleCell-2">
            <div className="listTitleCell-3">
              <span className="listTitleCell-4">
                {data.hyzd === 1
                  ? translate(getText("買多"))
                  : translate(getText("買空"))}
                /{data.buyprice}
              </span>
              <span className="listTitleCell-5">{data.num.toFixed(4)}</span>
              <span className="listTitleCell-6">
                <span
                  style={{
                    color: data.isWin === 1 ? "#23c363" : "red",
                  }}
                >
                  {data.isWin === 1
                    ? data.ploss.toFixed(2)
                    : -data.ploss.toFixed(2)}
                </span>
              </span>
            </div>
          </div>
        </div>
      );
      nodes.push(node);
    }
    //判断空
    if (nodes.length === 0) {
      nodes.push(<Empty description={translate(getText("暂无更多了"))} />);
    } else {
      nodes.push(
        <div className="levermingxi-43">{translate(getText("暂无更多了"))}</div>
      );
    }
    return nodes;
  };
  return (
    <div role="tabpanel" id="mingxi" className="levermingxi-1">
      <div className="listTitle-1">
        <div className="listTitle-2">
          <div className="listTitle-3">
            <span className="listTitle-4">
              {translate(getText("方向"))}/{translate(getText("持仓价"))}
            </span>
            <span className="listTitle-5">{translate(getText("數量"))}</span>
            <span className="listTitle-6">{translate(getText("盈亏"))}</span>
          </div>
        </div>
      </div>
      <div className="levermingxi-2">{getNode()}</div>
    </div>
  );
}
